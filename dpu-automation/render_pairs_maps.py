...
def main():
    ...
    ap.add_argument("--out-dir", help="Output maps directory")
    ap.add_argument("--valid", help="Valid label for titles")
    args = ap.parse_args()
    if args.nc:
        nc = Path(args.nc)
        out = Path(args.out_dir) if args.out_dir else Path("maps") / nc.parent.name
        meta = render_one(nc, out, args.valid or out.name)
